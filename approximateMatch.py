...
class ApproximateMatch(Callback):
    ...
    '''
    def __init__(self, valid_toks, valid_y, X_valid, padlen, idx2label, pred_dir='H:/twitter/model_output/predictions'):
...
class ApproximateMatch_SEQ(Callback):
    ...
    # Arguments
        ...
        pred_dir: path to the directory where this should write the predictions after
            each epoch, which will be scored
    '''
    def __init__(self, valid_toks, valid_y, valid_x, idx2label, pred_dir='H:/twitter/model_output/predictions', fileprefix=''):
        super(Callback, self).__init__()
        self.valid_toks = valid_toks
        self.valid_y = valid_y
